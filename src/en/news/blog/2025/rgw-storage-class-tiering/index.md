...
| **Target you define** | **Target data pool** | **Target Region**, **Location constraint**, **Endpoint**, **Access Key**, **Secret Key**, **Target Path**, and **Target storage class** |
...
- **Location constraint** (optional) — region used when RGW creates the target bucket on the remote S3 endpoint. For AWS, set this only if the region is other than US East (`us-east-1`); for example, `eu-north-1`.
- **Target storage class** (optional) — storage class objects are placed into on the remote endpoint (for example, `STANDARD`, `STANDARD_IA`, or `GLACIER`). If empty, the remote endpoint default is used (typically `STANDARD`).
...
- **Remote placement options**: **Location constraint** and **Target storage class** on Cloud S3 and Cloud S3 Glacier forms, for non-default AWS regions and remote storage classes.
