...
Contents:
- [Introduction: What is CBT (Ceph Benchmarking Tool)?  ](#intro)
- [What do you have to consider when you are benchmarking storage systems?  ](#consider)
- [What are you looking to achieve from the performance benchmark?](#achieve)
- [Starting up a ceph cluster for a performance benchmark](#start)

---

## <a id="intro"></a>Introduction: What is CBT (Ceph Benchmarking Tool)?  
...
## <a id="consider"></a>What do you have to consider when you are benchmarking storage systems?  
...
## <a id="achieve"></a>What are you looking to achieve from the performance benchmark?   
...
## <a id="start"></a>Starting up a ceph cluster for a performance benchmark  
...
Note: The above is using the development build containers. You can also pull released build containers (for Squid/Reef etc), from [here](https://quay.io/repository/ceph/ceph?tab=tags&tag=latest).
...
It's very important to take a note of the **volume name** and **pool name** you create, in my example above this is `test-image` and `rbd_replicated` respectively. As we are creating an erasure coded profile set up, we use the replicated pool name. (In [**part 2**](https://ceph.io/en/news/blog/2025/cbt-performance-benchmarking-part2/) within the `Benchmark Module` section you will need to refer to these names)
...
Now move onto [**Part 2**](https://ceph.io/en/news/blog/2025/cbt-performance-benchmarking-part2/) of the blog if you so wish, where you can take a look at defining a YAML file that will outline the workloads (tests) that you will be running on your ceph cluster!