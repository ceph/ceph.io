...
Only run "bare" `ceph-volume` commands if your cluster is not managed
by cephadm! Otherwise you’ll most likely end up with more chaos! 

## OSD FSID

In some failure scenarios you might need the FSID of each OSD to rebuild an OSD’s
directory structure. To collect those, you can also use ceph-volume or LV tags:
...
```
Only run “bare” ceph-volume commands if your cluster is not managed by cephadm!
Otherwise you’ll most likely end up with more chaos!
...
Chances are high that this approach won’t work in a scenario like we’re facing
...
Most likely you’ll have to enable the [dashboard](https://docs.ceph.com/en/latest/mgr/dashboard/#enabling)
again since all configurations are gone:
...
As I already pointed out before, don’t use the bare ceph-volume commands to try and fix things
